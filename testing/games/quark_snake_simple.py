#!/usr/bin/env python3
"""
Quark Snake Game - Simple Learning Demo

A simplified Snake game where Quark can easily learn to play.
Designed to show clear learning progress with visible improvements.
"""
...
class SimpleSnakeGame:
    """Simple Snake game environment for Quark to learn."""
    
    def __init__(self, width=600, height=400, grid_size=25):
        pygame.init()
        self.width = width
        self.height = height
        ...
        pygame.display.set_caption("Quark Snake - Simple Learning Demo!")
        ...
        self.epsilon = 0.5  # High exploration
        self.learning_rate = 0.2  # High learning rate
        self.gamma = 0.8  # Lower discount for immediate rewards
    ...
    def reset_game(self):
        ...
        self.score = 0
        self.steps = 0
        self.max_steps = 100  # Very short episodes for quick learning
    ...
    def get_state(self):
        """Get simple game state for learning."""
        head = self.snake[0]
        
        # Simple state: danger in 4 directions + food direction
        state = []
        ...
        # Food direction (simple: which quadrant)
        dx = self.food[0] - head[0]
        dy = self.food[1] - head[1]
        
        if abs(dx) > abs(dy):
            if dx > 0: state.append(1)  # Food to the right
            else: state.append(0)        # Food to the left
        else:
            if dy > 0: state.append(1)  # Food below
            else: state.append(0)        # Food above
    ...
    def get_reward(self, ate_food, steps_taken):
        """Calculate simple reward for the action."""
        if self.game_over:
            return -10  # Small death penalty
        
        if ate_food:
            return 50  # Big food reward
    ...
    def run_episode(self):
        ...
        while not self.game_over:
            ...
            # Calculate reward
            reward = self.get_reward(ate_food, self.steps)
            total_reward += reward
            ...
            time.sleep(0.02)  # Very fast episodes
    ...
    def learning_loop(self):
        ...
        while self.learning_active:
            ...
            avg_score = np.mean(self.episode_scores[-20:]) if self.episode_scores else 0
            ...
            if self.score > 0:
                self.epsilon = max(0.1, self.epsilon * 0.99)  # Faster decay when learning
            else:
                self.epsilon = max(0.1, self.epsilon * 0.999)  # Slower decay when stuck
    ...
    def draw_learning_info(self):
        ...
        if self.learning_progress:
            ...
            avg_text = self.font.render(f"Avg (20): {recent['avg_score']:.1f}", True, self.YELLOW)
        ...
        for i, instruction in enumerate(instructions):
            ...
            self.screen.blit(inst_text, (self.width - 200, 10 + i * 25))
    ...
    def save_learning_data(self):
        ...
        try:
            with open('testing/games/quark_snake_simple_learning_data.json', 'w') as f:
            ...
            if self.learning_progress:
                recent = self.learning_progress[-1]
                print(f"📈 Average score (last 20): {recent['avg_score']:.2f}")
                
                # Show learning progress
                if len(self.episode_scores) >= 10:
                    first_10_avg = np.mean(self.episode_scores[:10])
                    last_10_avg = np.mean(self.episode_scores[-10:])
                    improvement = last_10_avg - first_10_avg
                    print(f"📊 Learning Progress: {first_10_avg:.2f} → {last_10_avg:.2f} (Δ: {improvement:+.2f})")
...
def main():
    ...
    print("🐍 Starting Quark Snake Game - Simple Learning Demo!")
    print("🧠 Quark will learn to play Snake using simple reinforcement learning")
    print("🎮 Watch as Quark improves its strategy in real-time!")
    print("📊 Learning progress displayed live with clear metrics")
    print("\n🎯 Controls:")
