    ...
    def run_simulation(self, duration: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            while self.current_time < self.params.total_time and self.simulation_running:
                ...
                # Report progress
                progress_percent = (self.current_time / self.params.total_time) * 100
                if int(progress_percent) > last_reported_progress:
                    # Use carriage return to print on the same line
                    sys.stdout.write(f"\r⏳ Simulation Progress: {progress_percent:.1f}% complete ({self.current_time:.1f}/{self.params.total_time} hours)")
                    sys.stdout.flush()
            ...
            sys.stdout.write(f"\r⏳ Simulation Progress: 100.0% complete ({self.params.total_time:.1f}/{self.params.total_time} hours)\n")
            sys.stdout.flush()
