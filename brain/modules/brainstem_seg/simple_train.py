#!/usr/bin/env python3
"""
Simplified Brainstem Segmentation Training

Basic training loop without complex augmentations to demonstrate the pipeline.
"""
...
class SimpleBrainstemDataset:
    """Simplified dataset for training demonstration."""
    ...
    def _extract_patches(self):
        """Extract training patches with consistent sizes."""
        patches = []
            ...
            for y in range(0, w - self.patch_size[1] + 1, stride):
                for x in range(0, h - self.patch_size[0] + 1, stride):
                    ...
                    if np.sum(label_patch > 0) > 50:
                        patches.append((vol_patch, label_patch))
    ...
    def __getitem__(self, idx):
        volume, labels = self.patches[idx]
...
class SimpleTrainer:
    ...
    def __init__(self, data_dir: Path, output_dir: Path, device="cpu"):
        ...
        # Setup logging
        log_file = output_dir / f"simple_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        # Create dataset
        self.dataset = SimpleBrainstemDataset(data_dir)
        
        # Create model
        self.model = ViTGNNHybrid(
            input_channels=1,
    ...
    def train_epoch(self, epoch):
        ...
        accuracy = 100 * correct / max(total, 1)   # Avoid division by zero
...
def main():
    ...
    print("🧠 SIMPLIFIED BRAINSTEM SEGMENTATION TRAINING")
    print("=" * 60)
    
    # Paths
    data_dir = Path("/Users/camdouglas/quark/data/datasets/brainstem_segmentation")
    output_dir = Path("/Users/camdouglas/quark/data/models/brainstem_segmentation/simple")
    ...
    # Create trainer
    trainer = SimpleTrainer(data_dir, output_dir, device="cpu")
