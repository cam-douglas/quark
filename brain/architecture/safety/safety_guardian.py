...
class SafetyGuardian:
    ...
    def step(self, limbic_output: Dict[str, Any]) -> bool:
        ...
        if average_error > self.error_threshold:
            print("\n" + "="*80)
            print(f"!!! SAFETY GUARDIAN: EMERGENCY SHUTDOWN TRIGGERED !!!")
            print(f"    - Reason: Persistent high error signal detected.")
            print(f"    - Average error over last {self.window_size} steps: {average_error:.3f} > Threshold: {self.error_threshold:.3f}")
            print(f"    - This protocol prevents the agent from getting stuck in a suffering loop.")
            print("="*80 + "\n")
