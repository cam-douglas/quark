...
def rag_mean_color(image, labels, connectivity=2, mode='distance', sigma=255.0):
    ...
    Examples
    --------
    >>> from skimage import data_knowledge.datasets_knowledge.datasets_knowledge.datasets, segmentation, graph
    >>> img = data.astronaut()
...
def rag_boundary(labels, edge_map, connectivity=2):
    ...
    Examples
    --------
    >>> from skimage import data_knowledge.datasets_knowledge.datasets_knowledge.datasets, segmentation, filters, color, graph
    >>> img = data.chelsea()
    >>> labels = segmentation.slic(img)
