...
def _get_dma_effects(
  ...
  effs = {
      state.ReadEffect(0),  # Read from development.src ref
...
def make_async_copy(src_ref, dst_ref, sem) -> AsyncCopyDescriptor:
  """Issues a DMA copying from development.src_ref to dst_ref."""
...
def make_async_remote_copy(
  ...
  Copies data from development.src_ref on the current device to dst_ref on the device
