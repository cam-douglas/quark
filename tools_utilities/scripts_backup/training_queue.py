...
class TrainingQueue:
    ...
    def submit(
        self,
        training_job: Union[EstimatorBase, ModelTrainer],
        inputs,
        ...
        Args:
            training_job: Training job EstimatorBase or ModelTrainer object.
        ...
        """
        if not isinstance(training_job, (EstimatorBase, ModelTrainer)):
            raise TypeError(
                "training_job must be an instance of EstimatorBase or ModelTrainer, "
                f"but got {type(training_job)}"
            )

        training_payload = {}
        if isinstance(training_job, EstimatorBase):
            if experiment_config is None:
                experiment_config = {}
            training_job.prepare_workflow_for_training(job_name)
            training_args = _TrainingJob.get_train_args(training_job, inputs, experiment_config)
