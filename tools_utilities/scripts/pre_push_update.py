...
========================
1. **Regenerate `master_roadmap.md`** – scan every Markdown roadmap under
...
2. **Validate internal links** – fast check that all relative links inside
   `master_roadmap.md` resolve; abort push on breakage.

3. **Sync YAML task DB** – call `state.quark_state_system.task_loader` to align
   roadmap tasks with the YAML task store.

4. **Refresh README status** – update the Roadmap Status block in the root
   `README.md`.
